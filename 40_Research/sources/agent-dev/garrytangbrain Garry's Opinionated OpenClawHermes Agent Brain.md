...
Detailed setup paths (Postgres at scale, Supabase, thin-client mode) live in [`docs/INSTALL.md`](https://github.com/garrytan/gbrain/blob/master/docs/INSTALL.md).
...
Per-client guides (Claude Desktop, Code, Cursor, ChatGPT, Perplexity, Cowork) live under [`docs/mcp/`](https://github.com/garrytan/gbrain/blob/master/docs/mcp). HTTP server supports DCR-style client registration, scope-gated access (`read` / `write` / `admin`), and built-in rate limiting.
