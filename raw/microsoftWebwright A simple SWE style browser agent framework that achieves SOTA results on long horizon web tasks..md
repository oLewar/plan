---
title: "microsoft/Webwright: A simple SWE style browser agent framework that achieves SOTA results on long horizon web tasks."
source: "https://github.com/microsoft/Webwright"
author:
published:
created: 2026-05-27
description: "A simple SWE style browser agent framework that achieves SOTA results on long horizon web tasks.  - microsoft/Webwright: A simple SWE style browser agent framework that achieves SOTA results on long horizon web tasks."
tags:
  - "clippings"
---
## Webwright
...
webwright\_demo.mp4<video src="https://private-user-images.githubusercontent.com/17594221/588737230-4ed94cd5-11be-4daa-b2d7-1260a803baca.mp4?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..rHx95ULhvTv_BW7D-47FxxuOmeP7ACuXiuSBGfxFPFM" controls="controls"></video>
...
Webwright ships plugin manifests for both [Claude Code](https://docs.claude.com/en/docs/claude-code/plugins) ([`.claude-plugin/plugin.json`](https://github.com/microsoft/Webwright/blob/main/.claude-plugin/plugin.json)) and [OpenAI Codex](https://developers.openai.com/codex/plugins) ([`.codex-plugin/plugin.json`](https://github.com/microsoft/Webwright/blob/main/.codex-plugin/plugin.json)), with the shared skill at [`skills/webwright/`](https://github.com/microsoft/Webwright/blob/main/skills/webwright) and slash commands at [`skills/webwright/commands/`](https://github.com/microsoft/Webwright/blob/main/skills/webwright/commands). The host agent drives the Webwright loop natively — no extra LLM API key or cost beyond your host subscription. Hosts that read PNG screenshots natively skip the `image_qa` / `self_reflection` tools.
...
The `webwright` skill is now available to any OpenClaw agent surface (CLI, Telegram, etc.) — invoke it by asking the agent in natural language, or via the slash commands shipped under [`skills/webwright/commands/`](https://github.com/microsoft/Webwright/blob/main/skills/webwright/commands), e.g. `/webwright run <task>`.
...
Note: the named subcommands shipped under [`skills/webwright/commands/`](https://github.com/microsoft/Webwright/blob/main/skills/webwright/commands) (`/webwright:run`, `/webwright:craft`) are a Claude Code / Codex convention and are inert in Hermes; the skill itself still works end-to-end.
...
@misc{webwright2026,
  title        = {Webwright: A terminal is all you need for web agents},
         = {Lu, Yadong and Xu, Lingrui and Huang, Chao and Awadallah, Ahmed},
  year         = {2026},
