...
- Related: [[wiki/entities/owasp]], [[10_Reference/tools/owasp-agent-memory-guard]], [[wiki/concepts/causal-analysis]], [[wiki/concepts/efficiency-metric]], [[wiki/concepts/continual-harness]] (Prime Agent `/refine` is a first-class self-write; Factorio RCON cheat skills are the cautionary case), [[wiki/concepts/git-native-experiment-tree]] (OpenResearch freezes measured *code*, does not CRUD next-turn memory), [[wiki/concepts/self-healing-cdp-harness]] (writable `agent_helpers.py` is task code, not identity memory — still a write surface), [[wiki/concepts/mcp-tool-broker]] (same MCP family, opposite trust: AMG gates writes; HexStrike weaponizes CLIs)
